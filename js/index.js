...
var S = {
  init: function () {
    ...
    if (i !== -1) {
      ...
      S.UI.simulate("|#countdown 3||Selamat|Tahun|Baru|Misya||#rectangle|");
    }
...
S.Drawing = (function () {
    ...
    init: function (el) {
      ...
      window.addEventListener('resize', function (e) {
         S.Drawing.adjustCanvas();
          // Update matrix canvas too
  var matrixCanvas = document.getElementById('canvas-matrix');
  if (matrixCanvas) {
  ...
  if (typeof S.Shape !== 'undefined' && S.Shape.currentShape) {
    S.Shape.switchShape(S.Shape.currentShape, true);
   }
  },
...
S.ShapeBuilder = (function () {
  ...
  if (w < 480) return 8;  // Gap lebih kecil di mobile kecil
  if (w < 768) return 10; // Gap sedang di tablet
  return 13;              // Gap normal di desktop
})(),
  ...
  // Adjust fontSize for mobile devices
  if (a.w < 768) {
    baseFontSize = Math.min(fontSize * 1.5, 750); // Bigger on mobile
  } else if (a.w < 480) {
    baseFontSize = Math.min(fontSize * 2, 1000); // Even bigger on small mobile
  }

  setFontSize(baseFontSize);
  s = Math.min(baseFontSize,
    (shapeCanvas.width / shapeContext.measureText(l).width) * 0.8 * baseFontSize,
    (shapeCanvas.height / baseFontSize) * (isNumber(l) ? 1 : 0.45) * baseFontSize);
  setFontSize(s);
  ...
  return processCanvas();
    ...
    rectangle: function (w, h) {
      ...
      var settings = {
  particles: {
    length: window.innerWidth < 768 ? 300 : 500, // Kurangi particles di mobile
    duration: 2,
    velocity: window.innerWidth < 768 ? 80 : 100, // Velocity lebih lambat di mobile
    effect: -0.75,
    size: window.innerWidth < 768 ? 25 : 30, // Size lebih kecil di mobile
  },
};
      (
        function () {
  ...
  var scale = window.innerWidth < 768 ? 0.7 : 1; // Scale down on mobile
  return new Point(
